...
for(i in 1:length(points_per_game)){
  names(points_per_game[[i]]) = (names(player_info[i]))
}
...
```{r}
states = max_temps$state

CO = max_temps[states == "CO", ]
OH = max_temps[states=='OH', ]
VA = max_temps[states=='VA', ]
```
...
max_med = max(median_CO)
max_med_year = CO$year[which.max(median_CO)]


min_med = min(median_CO)
min_med_year = CO$year[which.min(median_CO)]
...
sum_dig = function(num){
  ...
  if(num == 0){
    ...
    return(last_dig + sum_dig(new_num))
  }
}
